...
class LeadGenerationCrawler:
    ...
    def export_to_csv(self, filename=None):
        ...
        # Prepare data for export - Only columns requested in assignment
        export_data = []
        ...
        print(f"   High priority (>85): {len(df[df['Probability'] >= 85])}")
        print(f"   Medium priority (70-84): {len(df[(df['Probability'] >= 70) & (df['Probability'] < 85)])}")
        print(f"   Low priority (<70): {len(df[df['Probability'] < 70])}")
